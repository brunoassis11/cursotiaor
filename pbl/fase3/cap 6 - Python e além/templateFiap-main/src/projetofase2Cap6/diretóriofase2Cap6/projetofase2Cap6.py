...
def adicionar_insumo_novo():
    ...
    while True:
        ...
        if unidade == "voltar":
            return
        validade = input_data("Validade (DD-MM-AAAA): ")
        if validade == "voltar":
            return
        ...
        if confirmar:
            insumo = {
                ...
                "historico": [
                    {"tipo": "entrada", "quantidade": quantidade, "data": datetime.now().strftime("%d-%m-%Y")}
                ]
            }
            ...
            if insumo['nome'].lower() == nome.lower():
                ...
                while True:
                    ...
                    if opcao == "0":
                        ...
                        nova_validade = input_data("Digite a nova validade (DD-MM-AAAA): ")
...
def gerar_relatorio_30_dias():
    ...
    for insumo in insumos:
        historico_recente = [h for h in insumo["historico"] if datetime.strptime(h["data"], "%d-%m-%Y") >= limite]
